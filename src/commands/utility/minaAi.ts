        ...
        options: [
          {
            name: 'include-global',
            description:
              'Include memories from DMs (default: only this server)',
...
async function handleMemories(interaction: ChatInputCommandInteraction) {
  try {
    ...
    const includeGlobal =
      interaction.options.getBoolean('include-global') ?? false
    ...
    if (memories.length === 0) {
      const embed = new EmbedBuilder()
        ...
        .setFooter({
          text: 'Use /mina-ai forget-me to delete memories anytime',
        })
    ...
    logger.log(
      `User ${userId} viewed ${memories.length} memories in guild ${guildId}`
    )
  } catch (error) {
    logger.error(
